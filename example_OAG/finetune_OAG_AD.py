...
for i, j in graph.node_feature['author'].iterrows(), total = len(graph.node_feature['author']):
    if i in author_dict:
        name_count[j['name']] += [i]
...
'''
gnn =  GNN(conv_name = 'hgt', in_dim = len(graph.node_feature['paper']['emb'].values[0]) + 401, \
